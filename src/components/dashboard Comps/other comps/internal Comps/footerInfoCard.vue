...
const localData = ref({
  getFooterHook: store.getFooterHook,
  getFooterCopyrights: store.getFooterCopyrights,
});
...
const profileFields = computed(() => [
  {
    label: 'Footer Hook:',
    key: 'getFooterHook',
    placeholder: 'footer hook',
  },
  {
    label: 'Footer CopyRights:',
    key: 'getFooterCopyrights',
    placeholder: 'footer copyRights',
  },
]);
...
const saveEdits = (id) => {
  status.value = true;
  confSave.value = true;
  store.setFooterHook(localData.value.getFooterHook);
  store.setFooterCopyrights(localData.value.getFooterCopyrights);
  saveToDb();
};
...
<template>
  <div class="contactInfo">
    <SubHeader heading="Footer section" class="flex-1" />
    <div
