...
const localData = ref({
  getHeroDescription: store.getHeroDescription,
});

const profileFields = computed(() => [
  {
    label: 'Hero Description:',
    key: 'getHeroDescription',
...
const saveEdits = (id) => {
  status.value = true;
  confSave.value = true;
  store.setHeroSectionDescription(localData.value.getHeroDescription);
  saveToDb();
};
...
<template>
  <div class="heroInfo">
    <SubHeader heading="Hero section" class="flex-1" />
    <div
